module spring.bind.commands_struct;
...
struct SInitPathCommand
{
	...
	const(float)* start_posF3;
	/// The goal location of the requested path
	const(float)* end_posF3;
...
struct SAddLineDrawCommand
{
	...
	const(float)* posFrom_posF3;
	/// to this pos, again only x and z matter
	const(float)* posTo_posF3;
...
{
	const(float)* pos1_posF3;
	const(float)* pos2_posF3;
	const(float)* pos3_posF3;
	const(float)* pos4_posF3;
	float width;
...
{
	const(float)* rayPos_posF3;
	const(float)* rayDir_posF3;
